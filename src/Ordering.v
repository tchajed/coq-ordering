...
Local Tactic Notation "gen" constr(X1) :=
  generalize dependent X1.
Local Tactic Notation "gen" constr(X1) constr(X2) :=
  gen X2; gen X1.
Local Tactic Notation "gen" constr(X1) constr(X2) constr(X3) :=
  gen X3; gen X1 X2.
Local Tactic Notation "gen" constr(X1) constr(X2) constr(X3) constr(X4) :=
  gen X4; gen X1 X2 X3.
Local Tactic Notation "gen" constr(X1) constr(X2) constr(X3) constr(X4) constr(X5) :=
  gen X5; gen X1 X2 X3 X4.
Local Tactic Notation "gen" constr(X1) constr(X2) constr(X3) constr(X4) constr(X5) constr(X6) :=
...
Section ThreeListInduction.
  ...
  Definition zip3 (l1: list A) (l2:list B) (l3:list C) : list (A*B*C) :=
    zip (zip l1 l2) l3.
...
Local Lemma cmp_list_eq A {ord: Ordering A} x y :
    cmp_list ord x y = Eq <-> x = y.
Proof.
